	...
	t.Run(
		...
		func(t *testing.T) {
			...
			faked_getTargetNodes := func(tags map[string]string, selectFunc func(nodes []Node) int) []Node {
				...
				sort.Slice(
					allNodes,
					func(i, j int) bool { return allNodes[i].Name < allNodes[j].Name },
				)
...
func Test_filterArray(t *testing.T) {
	...
	filtertests := []struct {
		...
		{"Empty input returns empty output", []Node{}, matchAll, []Node{}},
		{
			"All but first match",
			[]Node{n1, n2, n3},
			func(m Node) bool { return m.Name != "node1" },
			[]Node{n2, n3},
		},
		{
			"All but last match",
			[]Node{n1, n2, n3},
			func(m Node) bool { return m.Name != "node3" },
			[]Node{n1, n2},
		},
		{
			"Middle does not match",
			[]Node{n1, n2, n3},
			func(m Node) bool { return m.Name != "node2" },
			[]Node{n1, n3},
		},
		{
			"Only middle matches",
